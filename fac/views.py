...
class ClienteView(SinPrivilegios, \
    generic.ListView):
...
class VistaBaseEdit(SuccessMessageMixin, SinPrivilegios, \
    ...
    context_object_name = "obj"
    success_message = "Registro Editado"

    def form_valid(self, form):
        form.instance.um = self.request.user.id
        return super().form_valid(form)

#heredad de las clases
...
def clienteInactivar(request,id):
    ...
    if request.method=="POST":
        #si cliente se logró crear
        if cliente:
            #volver al estado contrario
            cliente.estado = not cliente.estado
            cliente.save()
            #enviar ok al front end
...
def facturas(request,id=None):
    ...
    detalle = {}
    #filtrar los clientes que tengan estado true
    clientes = Cliente.objects.filter(estado=True)
    #inicializar contexto con 3 variables
    ...
    if request.method == "POST":
        ...
        #si no se envia el id, es por que es una factura nueva
        if not id:
            enc = FacturaEnc(
                cliente = cli,
                fecha = fecha
...
class ProductoView(inv.ProductoView):
    #redireccionar 
    template_name="fac/busqueda_por_producto.html" 


def anular_detalle_fac(request, id):
    template_name = "fac/factura_anular_detalle.html"
